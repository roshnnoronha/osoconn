...
The design is as per the thick plate philosophy 
described in AISC design guide 4 where the 
thckness of the plate is selected to preclude 
prying action in bolts.This leads to thicker 
plates and smaller bolts. The connection is 
...
private:	
	...
	double tcs;									//Thickness of column stiffener plate
	weld wld;
	...
	double ts_req_b;							//Required minimum thickness of stiffener plate [eq 3.16, AISC design guide 4]
	double ratio_pl;
	...
	double Yc;									//Column yield line parameter [AISC design guide 4]
...
momConnection1::momConnection1(std::map<std::string,std::string> data):plate(section_property::MEMB_TYPE_PLATE){	
	...
	param("CONNECTION_CODE").addAllowableValue("MC001");
	...
	std::string connDesc = data["CONNECTION_CODE"];
...
void momConnection1::calc(){
	...
	if (connectionType!=CONN_4E){
		...
		ts_req=tw_beam*(Fyb/Fyp);						//[eq 3.15, AISC design guide 4]
	
		//Stiffener Local Buckling
		hst=(dp-d_beam)/2.0;							//Stiffener plate height
		ts_req_b=1.79*hst*sqrt(Fyp/E);					//Required minimum thickness of stiffener plate [eq 3.16, AISC design guide 4]
	}
	//Bolt tension check
	...
	lci = sb - dbh;
	Rn_bbo = min(1.2*lco*tp*Fup,2.4*db*tp*Fup);
	Rn_bbi = min(1.2*lci*tp*Fup,2.4*db*tp*Fup);
	if (connectionType == CONN_8ES)
		Rn_bb = 2*Rn_bbo + 6*Rn_bbi;
	else	
		Rn_bb = 2*Rn_bbo + 2*Rn_bbi;
	if (method==LRFD)
	...
	if (connectionType == CONN_8ES)
		Rn_bbc = 8 * min(1.2*sb*tf*Fuc,2.4*db*tf*Fuc);
	else
		Rn_bbc = 4 * min(1.2*sb*tf*Fuc,2.4*db*tf*Fuc);
	if (method==LRFD)
	...
	//Web to plate weld check	
	lw = (d_beam - 2*k_beam)*2;
	...
	tw_min1 = (d_beam - tf_beam + d - 2*tf)/90;
	tw_min = (d - 2*k)*sqrt(Fyc) / 418;
	tw_min = tw_min > tw_min1? tw_min:tw_min1;
	ratio_twc = tw_min / tw;

	//Column Flange Yielding
	Yc=columnYieldLineParameter();						//Column yield line parameter [AISC design guide 4]
	if (method==LRFD)
		tf_req=sqrt(1.11*phi_br*M_np/(phi_b*Fyc*Yc));	//[eq 3.20, AISC design guide 4]
	else	
...
/*	
	Column Web Buckling
	NOTE: Only required for interior columns
*/
	...
	if (col_doubler_plate){
		...
		tdp_req = Vu_dp/(0.9*0.6*Fyp * d);
		twld_dp_req = 1.7 * Fyp*tdp_req/Fuw;
		ratio_wdf = twld_dp/twld_dp_req;
	}
	...
	ratio_max = ratio_ww > ratio_max?ratio_ww:ratio_max;							//interaction ratio for weld at web
	if (connectionType != CONN_4E)
	...
	if ((colStiffenerType == COL_UNSTIFFENED) && (!col_doubler_plate)){
	...
	if (!col_doubler_plate)
		ratio_max = ratio_twc > ratio_max?ratio_twc:ratio_max;						//interaction ratio in column web panel shear
...
double momConnection1::plateYieldLineParameter()
{
	...
	double d_e=0.5*(dp-d_beam)-pfo;					//Bolt edge distance	
	if (connectionType==CONN_4E){
		double h0=d_beam - 0.5*tf_beam + pfo;
		double h1= d_beam - tf_beam - pfi;			
		...
		double h1= d_beam - 0.5*tf_beam + pfo + pb;
		double h2= d_beam - 0.5*tf_beam + pfo;
		double h3= d_beam - tf_beam - pfi;
		double h4= d_beam - tf_beam - pfi -pb;
		if (d_e < s)
...
double momConnection1::columnYieldLineParameter()
{
	...
	double psi=pfi+0.5*(tf_beam-tcs);				//Distance of the inner bolt from the column striffener plate edge
	double pso= pfo+0.5*(tf_beam-tcs);				//Distance of the outer bolt from the column stiffener plate edge
...
void momConnection1::printDesignOutput ()
{
...
std::string momConnection1::designSummary()
{
	...
	if (ratio_ww > 1) _p("","Fails in web weld strength");
	if (connectionType != CONN_4E)
		if (ratio_ws > 1) _p("","Fails in stiffener to beam weld strength");
	...
	if (!col_doubler_plate)
		if (ratio_twc > 1) _p("","Fails in column web panel shear");
	if (colStiffenerType == COL_STIFFENED){
...
std::string momConnection1::designCalculation()
{
	...
	_h3("Beam Web to End Plate Weld Check");
		...
		_p(ratio_ww,"Interaction ratio for beam web weld","Vu/Ra","");	
	...
	_p(Pt, "Bolt tensile strength (Pt)","",_force);
		...
		_p(tp_req/tp, "Interaction ratio for plate thickness","tp`/tp","");
	...
	if (connectionType != CONN_4E){
		_h3("End Plate Stiffener Thickness Check");
			_p(ts_req,"Required minimum stiffener thickness","ts1`=tw*(Fyb/Fyp)", _length);
			_p(hst,"Stiffener plate height (hst)","",_length);
			_p(ts_req_b,"Required minimum thickness of stiffener plate for buckling","ts2`=1.79*hst*sqrt(Fyp/E)" , _length);
			if (ts_req_b> ts_req)
				_p(ts_req_b/ts, "Interaction ratio for stiffener thickness","max(ts1`,ts2`)/ts","");
			else
				_p(ts_req/ts, "Interaction ratio for stiffener thickness","max(ts1`,ts2`)/ts","");
	...
	if(colStiffenerType == COL_STIFFENED){
		...
		_h3("Transverse stiffener shear strength");
			...
			_p(ratio_cst,"Interaction ratio for stiffener in shear","Ru/(2*Ra)","");

		_h3("Transverse Stiffener Thickness Check");
			_p(tst_min, "Minimum required thickness of stiffener (ts`)","",_length);
			_p(ratio_tst,"Interaction ratio for stiffener thickness", "ts`/ts","");
		...
		_h3("Transverse Stiffener to Web Weld Check");
			...
			if (method==LRFD){
				_p(phi_w, "LRFD factor for weld strength (phi)", "", "");
				_p(Ra_wsw,"Stiffener to web weld capacity","Ra=Rn*phi",_force);
			}else{
				_p(omega_w,"ASD factor weld strength (omega)","" , "");
				_p(Ra_wsw, "Stiffener to web weld capacity","Ra=Rn/omega" , _force);	
			}		
	...
	if(col_doubler_plate){
		...
		_h3("Doubler Plate Thickness Check");
			...
			_p(twld_dp_req, "Required plate to flange weld thickness","",_length);
			_p(ratio_wdf,"Interaction ratio for plate to weld thickness","","");
	}
