...
private:
	...
	const double phi_bo=0.75;					//LRFD factor for bolt shear
	const double omega_bo=2.0;					//ASD factor for bolt shear
	...
	double kb;									//buckling factor for gusset
	...
	double bw;									//length of horizontal run of weld
	...
	double fw;									//maximum stress in weld
	double Rn_w;								//Nominal strength of weld
	double Ra_w;								//allowable strength
	double UR_w;								//interaction ratio for weld check
	...
	double ft_wbm;														//maximum stress transverse to the weld
...
brace(section_property::MEMB_TYPE_L),clip2(section_property::MEMB_TYPE_L),clip3(section_property::MEMB_TYPE_L),gusset(section_property::MEMB_TYPE_PLATE){
	...
	param("CONNECTION_CODE").addAllowableValue("VB001");
	...
	std::string connID = data["CONNECTION_ID"];
	if (data["METHOD"] == "LRFD"){
		method = LRFD;
	...
	if (data["FAYING_SURFACE"] == "CLASS_A")
	...
	w3 = w2;
	a2 = atoi(data["GUSSET_CLIP_LOCATION"].c_str());
...
void vbConnection1::checkFitup(){
	...
	if (li - g < ed_min) err.addError("Inadequate edge distance perpendicular to brace. Try reducing bolt gage on brace.");
...
void vbConnection1::calc(){	
	...
	An_br = Ag_br-2.0*ta_br*dbh;									//Net cross-section area of brace
	...
	//Bolt shear strength
	Pb_br = P_br/nb;												//shear per bolt
	Rn_bo = blt.nominalShearStrength(2);							//Nominal shear strength of a single bolt with 2 slip/shear plane
	Ra_bo = allowableStrength(Rn_bo,phi_bo,omega_bo);				//Capacity of bolt in shear
	...
	Rn1_bl = (0.6*Fu_b*Anv + 0.5*Fu_b*Ant);							//Nominal strength in block shear of brace
	Rn2_bl = (0.6*Fy_b*Agv + 0.5*Fu_b*Ant);	
	...
	if (nr == 2){
		...
		Ant_g = (sb - dbh)*tg;
		Rn1_bl_g = (0.6*Fu_g*Anv_g + 0.5*Fu_g*Ant_g);				//Nominal strength in block shear of gusset
		Rn2_bl_g = (0.6*Fy_g*Agv_g + 0.5*Fu_g*Ant_g);	
	...
	lgw = 2.0 *(nb -1.0) *sb *tan(30.0 *PI /180.0);					//Length of Whitmore section
	...
	//Gusset compression buckling strength (new)
	...
	//Beam to column connection checks
	T = (TF_b + Hc);
	Tb = T/(2*nb);														//Tension per bolt
	R = (TF_b + Hc);
	Rb = R/(2*nb);														//Shear per bolt
	...
	//Bolt bearing at column flange
	lc_bco = (sb-dbh);													//clear distance between the edge of bolt and the edge of adjacent bolt
	Rn_bco = 1.2*lc_bco*tf_co*Fu_c;										//nominal strength of bolt group in bearing
	Rn1_bco = 2.4*db*tf_co*Fu_c;
	...
	g1 = gb/2.0 - tw_b/2.0 - ta_clip;									//distance of face of angle to bolt center
	p_ed = min(ed3,g1) + min(sb/2.0,g1);								//pitch of edge bolt
	p_mid = min(sb, g1*2.0);											//pitch of mid bolt
	p = min(p_ed, p_mid);												//minimum pitch distance
	B = Ra_bt;															//available tension strength per bolt
	a = lo_clip - (gb/2.0 - tw_b/2.0);									//distance of bolt centerline to edge of fitting
	b = gb/2.0 - tw_b/2.0 - ta_clip/2.0;								//distance of bolt centerline to centerline of clip angle
	a1 = a + db/2.0;
	if (a1 > (1.25*b + db/2.0)) 
		a1 = 1.25*b + db/2.0;
	b1 = b-db/2.0;
	...
	b_co = (gb - tw_co) / 2.0;
	b1_co = b_co - db / 2.0;
	p_eff = ((nb - 1)*sb + PI*b_co + 2*a) / nb;							//effective pitch at column
	delta_co = 1.0 - dbh / p_eff;										//ratio of net length at bolt line to gross length at face of stem
	a_co = min((bf_co - gb) / 2.0, (2*lo_clip +tg -gb)/2.0);
	...
	alpha1_co = (alpha1_co>1) ? 1 : alpha1_co;	
	if (alpha1_co < 0.0)	
		Q_co = 1;
	else	
	...
	//Weld shear strength
	bw = li_clip-c;																				//length of horizontal run of weld
	x = (bw*bw)/(2.0*bw + l2);																	//cetroid of weld group
	wl = 2.0*bw + l2;																			//total weld length
	Ip = (l2*l2*l2)/12.0 + bw*(l2*l2)/2.0 + 2*(bw*bw*bw)/3.0 - (bw*bw*bw*bw)/(2.0*bw + l2);		//polar moment of inertia of weld group
	e = li_clip-x;																				//eccentricity of force
	fx = T/(2.0*wl) + (R*e)*l2/(4.0*Ip);														//horizontal component of the stress
	fy = R/(2.0*wl) + (R*e)*(bw-x)/(2.0*Ip);													//vertical component of the stress
	fw = sqrt(fx*fx + fy*fy);																	//maximum stress in weld
	Rn_w = 0.707*w2*0.6*Fu_w;																	//Nominal strength of weld
	Ra_w = allowableStrength(Rn_w,phi_w,omega_w);												//allowable strength
	UR_w = fw / Ra_w;
	
	//Beam web rupture at weld (new)
	Rn_rbw = 0.6*Fu_bm*tw_b;
	Ra_rbw = allowableStrength(Rn_rbw,phi_r,omega_r);
	UR_rbw = 2*fw/Ra_rbw;
	
	//Block shear at clip angle check (new)
	Agv_blc = 2*ta_clip*(n3 - 1)*sb + ed2;
	Anv_blc = 2*Agv_blc - (n3 - 0.5)*dbh*ta_clip;
	Ant_blc = 2*ta_clip*(lo_clip - (gb - tw_b)/2);
	...
	Rn_ycl = 2.0*0.60*Fy_a*ta_clip*l2;									//eqn. J4-3, AISC 360-14
	Ra_ycl = allowableStrength(Rn_ycl, phi_y, omega_y);					//allowable strength
	UR_ycl = sqrt(R*R + T*T)/Ra_ycl;									//interaction ratio in clip shear yielding

	//Shear rupture of clip angle
	Anet_cl = 2.0*(ta_clip*l2 - nb*dbh*ta_clip);						//net area in shear 
	...
	ft_wbm=Vb/(2.0*lgb)+3*Mb/(lgb*lgb);									//maximum stress transverse to the weld
	fl_wbm=Hb/(2.0*lgb);												//stress logitudinal to the weld
	fw_wbm=sqrt(ft_wbm*ft_wbm+fl_wbm*fl_wbm);							//maximum resulting stress in weld
	...
	//Beam web yielding
	Rn_wyb = Fy_b*tw_b*(2.5*k_b + lgb);									//nominal strength in web yielding
	...
	//Beam web crippling
	if (lgb/d_b <= 0.2)
		Rn_wcb = 0.40*tw_b*tw_b*(1.0 + 3.0*lgb/d_b*pow(tw_b/tf_b, 1.5))*sqrt(E*Fy_b*tf_b/tw_b);
	else
		Rn_wcb = 0.40*tw_b*tw_b*(1.0 + (4.0*lgb/d_b - 0.2)*pow(tw_b/tf_b, 1.5))*sqrt(E*Fy_b*tf_b/tw_b);
	...
	Ra_bv_2 = allowableStrength(Rn_bv_2,phi_bv,omega_bv);				//allowable strength in bolt shear
	...
	//Clip angle prying	check
	g1_2 = gb/2 - tg/2 - ta_clip;										//distance of face of angle to bolt center
	p_ed_2 = min(ed3,g1_2) + min(sb/2,g1_2);							//pitch of edge bolt
	p_mid_2 = min(sb, g1_2*2);											//pitch of mid bolt
	p_2 = min(p_ed_2, p_mid_2);											//minimum pitch distance
	B_2 = Ra_bt_2;														//available tension strength per bolt
	a_2 = lo_clip - (gb/2 - tg/2);										//distance of bolt centerline to edge of fitting
	b_2 = gb/2 - tw_b/2 - ta_clip/2;									//distance of bolt centerline to centerline of clip angle
	a1_2 = a_2 + db/2.0;
	...
	alpha1_2 = (alpha1_2>1) ? 1 : alpha1_2;
	if (alpha1_2 < 0.0)	
		Q_2 = 1;
	else	
	...
	p_eff_2 = ((n2 - 1)*sb + PI*b_co_2 + 2*a_co_2) / n2;				//effective pitch at column
	delta_co_2 = 1.0 - dbh / p_eff_2;									//ratio of net length at bolt line to gross length at face of stem
	rho_co_2 = b1_co_2 / a1_co_2;
	if (method == LRFD)
		tc_co_2 = sqrt((4.0 * B * b1_co_2) / (phi_f * p_eff_2 * Fu_c));
	else
		tc_co_2 = sqrt((omega_f * 4.0 *B * b1_co_2)/(p_eff_2 * Fu_c));
	tr_co_2 = tc_co_2 / tf_co;
	alpha1_co_2 = 1/(delta_co_2*(1+rho_co_2)) * (tr_co_2*tr_co_2 - 1);
	alpha1_co_2 = (alpha1_co_2>1) ? 1 : alpha1_co_2;	
	if (alpha1_co_2 < 0.0)	
		Q_co_2 = 1;
	else	
	...
	Ip_2 = (l_wld_2*l_wld_2*l_wld_2)/12.0 + b_wld_2*b_wld_2*(l_wld_2+b_wld_2)*(l_wld_2+b_wld_2)/l_wld_2 ;	//polar moment of inertia of weld group
	ecc_2 = li_clip-x_wld_2;																				//eccentricity of force
	fx_2 = T2/(2.0*l_wld_2) + (R2*ecc_2)*l_wld_2/(4.0*Ip_2);												//horizontal component of the stress
	...
	//Gusset rupture at weld check (new)
	Rn_rbw_2 = 0.6*Fu_g*tg;
	Ra_rbw_2 = allowableStrength(Rn_rbw_2, phi_r,omega_r);				
	UR_rbw_2 = 2*fw_2/Ra_rbw_2;
	
	//Clip angle block shear check (new)
	Agv_blc_2 = 2*ta_clip*(n2 - 1)*sb + ed2;
	Anv_blc_2 = 2*Agv_blc_2 - (n2 - 0.5)*dbh*ta_clip;
	Ant_blc_2 = 2*ta_clip*(lo_clip - (gb - tg)/2);
	...
	Ra_ycl_2 = allowableStrength(Rn_ycl_2,phi_y,omega_y);				//allowable strength
	...
	UR_max = UR_w > UR_max ? UR_w : UR_max;									//interaction ratio for weld check
...
void vbConnection1::printDesignOutput(){
...
std::string vbConnection1::designSummary(){
	...
	if (UR_w > 1) _p("","Fails in weld at beam to column connection");
...
std::string vbConnection1::designCalculation(){
	...
	_h3("Bolt shear check at brace");
		...
		if (method == LRFD){
			_p(phi_bo, "LRFD factor in bolt shear (phi)");
			_p(Ra_bo, "Allowable strength of bolt in shear","Ra=Rn*phi",_force);
		}else{
			_p(omega_bo, "ASD factor in bolt shear (omega)");
			_p(Ra_bo, "Allowable strength of bolt in shear","Ra=Rn/omega",_force);
		}
		_p(UR_bo, "Interaction ratio for bolt shear","P/Ra","");
	...
	_h3("Bolt bearing at brace check");
		...
		_p(UR_bbr, "Interation ratio for bolt bearing at brace","P/Ra","");
	...
	_h3("Bolt bearing at gusset check");
		...
		_p(UR_bg, "Interaction ratio for bolt bearing at gusset","P/Ra","");
	...
	_h3("Gusset to beam weld check");
		_p(fw_wbm, "Maximum stress in weld (fw)","",_force + "/" + _length);
	...
	_h3("Beam web shear check");
		_p(Rn_wsb, "Nominal strength of beam in web shear (Rn)","",_force);
		if (method == LRFD){
			_p(phi_ws,"LRFD factor for web shear (phi)");
			_p(Ra_wsb, "Allowable strength of beam in web shear","Ra=Rn*phi",_force);
		}else{
			_p(omega_ws,"ASD factor for web shear (omega)");
			_p(Ra_wsb, "Allowable strength of beam in web shear","Ra=Rn/omega",_force);
		
		}
		_p(UR_wsb, "Interaction ratio in web shear","(Vb+4*Mb/lg)/Ra");
	...
	_h3("Bolt shear check");
		...
		_p(UR_bv,"Interation ratio in bolt shear","R1/Ra");
	...
	_h3("Weld check");
		_p(Ip, "Polar moment of inertia of weld group","",_length + "^3");
		_p(fw, "Maximum stress in weld (fw)","",_force + "/" +_length);
		_p(Rn_w, "Nominal strength of weld (fn)","",_force + "/" + _length);
		if (method == LRFD){
			_p(phi_w, "LRFD factor for weld strength (phi)");
			_p(Ra_w, "Allowable strength of weld","fa=fn*phi",_force + "/" + _length);
		}else{
			_p(omega_w, "ASD factor for weld strength (omega)");
			_p(Ra_w, "Allowable strength of weld","fa=fn/omega",_force + "/" + _length);		
		}
		_p(UR_w, "Interaction ratio in weld strength","fw/fa");
	...
	_h3("Block shear at clip angle check");
		...
		_p(UR_bl, "Interaction ratio in block shear at clip angle","(Rb+Vb)/Ra","");
	...
	if (supp_type == COLUMN_FLANGE){
		...
		_h3("Column web shear check");
			_p(Rn_wsc,"Nominal strength of column in web shear (Rn)","",_force);
			if (method == LRFD){
				_p(phi_ws, "LRFD factor in web shear (phi)");			
				_p(Ra_wsc, "Allowable strength of column in web shear","Ra=Rn*phi",_force);
			}else{
				_p(omega_ws, "ASD factor in web shear (omega)");
				_p(Ra_wsc, "Allowable strength of column in web shear","Ra=Rn/omega",_force);
			}
			_p(UR_wcc,"Interaction ratio in column web shear","Hc/Ra");
	}
	...
	_h3("Bolt shear check");
		...
		_p(UR_bv_2,"Interation ratio in bolt shear","R2/Ra");
