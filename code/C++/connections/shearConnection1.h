...
shearConnection1::shearConnection1(std::map<std::string,std::string>& data):angleSection(section_property::MEMB_TYPE_L){
	...
	param("CONNECTION_CODE").addAllowableValue("SC001");
	...
	db = blt.diameter();
	std::string boltType = data["BOLT_TYPE"];
	if (boltType == "BEARING")
...
void shearConnection1::checkFitup(){
	...
	if (us.getUnitSystem() == unit_system::NEWTON_MM)
		...
		gap = 0.1;
	if (ct < (dct - wt - gap)) err.addError("Insufficient space for weld at top of clip angle. Try lowering the clip angle."); 
	//angle botton from bottom cope
	if ((ct + la) > (b_d - dcb - wt - gap)) err.addError("Insufficient space for weld at bottom of clip angle. Try raising the clip angle or reducing the length of the angle.");
...
void shearConnection1::calc(){
	...
	lc_s = sbc;
	if (supp_type == 1){												//beam 
		ts = b_tw;														//thickness of supporting member
	...
	Rn_bbs=1.2*lc_s*ts*fu_b;											//nominal strength in bolt bearing at support
	Rn1_bbs=2.4*db*ts*fu_b; 
	...
	etf = abs(xc-xw);													//eccentricity of transfer load from centroid of cope
	...
	g1 = gb/2.0 - b_tw/2.0 - ta;										//distance of face of angle to bolt center
	p_ed = min(ed,g1) + min(sb/2.0,g1);									//pitch of edge bolt
	p_mid = min(sb, g1*2.0);											//pitch of mid bolt
	p = min(p_ed, p_mid);												//minimum pitch distance
	B = Ra_bt;															//available tension strength per bolt
	a = lo - (gb/2.0 - b_tw/2.0);										//distance of bolt centerline to edge of fitting
	b = gb/2.0 - b_tw/2.0 - ta/2.0;										//distance of bolt centerline to centerline of clip angle
	a1 = a + db/2.0;
	if (a1 > (1.25*b + db/2.0)) 
		a1 = 1.25*b + db/2.0;
	b1 = b - db/2.0;
...
void shearConnection1::printDesignOutput()
{
...
std::string shearConnection1::designCalculation()
{
	...
	_h3("Bolt Shear Check");
		...
		_p(UR_bv,"Utility ratio in bolt shear","Vb/Ra");
	...
	_h3("Bolt Bearing at Clip Angle Check");	
		...
		_p(UR_bb,"Utility ratio in bearing at clip angle","Vb/Ra");	
	...
	_h3("Bolt Bearing at Support Check");	
		...
		_p(UR_bbs,"Utility ratio in bearing at support","Vb/(Ra*r)");
	...
	_h3("Bolt Tension Check (without prying");	
		...
		_p(UR_bt,"Utility ratio in bolt tension","Tb/B");
	...
	_h3("Weld Check");
		...
		_p(UR_w,"Utility ratio for weld", "fw/Ra");	
	
	_h3("Web Rupture at Weld Check");	
		_p(tw_min,"Minimum thickness of web at weld (tw`)","",_length);
		_p(UR_wr,"Utility ratio in rupture at weld","tw`/tw");
	...
	_h3("Clip Angle Shear Yielding Check");
		...
		_p(UR_vyc,"Utility ratio in shear yielding", "Va/Ra");
	...
	_h3("Clip Angle Shear Rupture Check");	
		...
		_p(UR_vrc,"Utility ratio in shear rupture","Va/Ra");
	...
	if ((dct != 0) || (dcb != 0)){
		_h3("Beam Cope Flexure Check");
			...
			_p(UR_cf,"Utility ratio in coped section flexure","M/Ma");
		...
		_h3("Beam Cope Compression Check");	
			...
			_p(UR_ca,"Utility ratio in coped section compression","TF/Pa");	
		...
		_h3("Beam Cope Shear Check");	
			...
			_p(UR_cv,"Utility ratio in coped section shear","SF/Ra");
	}
	return o;
}
