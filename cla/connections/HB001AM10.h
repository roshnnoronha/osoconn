...
public:
    ...
    enum joint_type {BEAM_JOINT, COLUMN_JOINT};
	HBConnection1();
...
private:
	...
	const double phi_rg=0.75;	//LRFD factor in gusset yielding
	const double omega_rg=2.0;	//ASD fctor in gusset yielding
    ...
    double extension_c1;		//Gusset extension from beam centerline at connection 1 (for bottom flange connection only)
    double extension_c2;		//Gusset extension from beam centerline at connection 2 (for bottom flange connection only)
    ...
    double gage_bm1;            //bolt gage of beam 1 flange
    double gage_bm2;             //bolt gage of beam 2 flange   
	weld wld;
    ...
    double d_col;               //depth of column section
	double tw_col;              //web thickness of column section
	double tf_col;              //flange thickness of column section
	double bf_col;              //flange width of column section    
    int col_orientation;        //column orientation angle of web with centerline of beam 1 - either 0 or 90
    
	double cope_bm1;            //cope length of beam 1 
    double setback_bm1;         //setback of beam at connection 1   
    double cope_bm2;            //cope length of beam 2
    double setback_bm2;         //setback of beam at connection 2
	double a;					//TOS?
    ...
    //variables for gusset tension rupture strength
    double An_g;                //net area of gusset in tension
    double Pn_rg;               //nominal strength of gusset in rupture
	double cap_rg;              //gusset rupture capacity
	double ratio_rg;            //interaction ratio in gusset tension rupture
    ...
    double ed_b1;               //bolt edge distance on beam 1
    ...
    double gus_in_ed_2;         //gusset inner edge distance from beam centerline
    double cutout_dim_1;        //cutout dimension where block shear is measured
    ...
    //variable for shear tab flexure yielding check at connection 1
    double Mn_fe_c1;            //nominal flexural strength of shear tab
    double cap_fe_c1;           //shear tab flexural strength  
    double ratio_fe_c1;         //interaction ratio in shear tab flexure	
    ...
    //variables for gusset flexure yielding check at connection 1
    double ec_c1;               //eccentricity of force at connection 1
    double Mn_f_c1;             //Nominal flexure strength of gusset at connection 1
    double cap_f_c1;            //Gusset plate flexure strength at connection 1
	double ratio_f_c1;          //interaction ratio in gusset flexure at connection 1
    ...
    double ed_b2;               //bolt edge distance on beam 2
    ...
    double gus_in_ed_1;         //gusset inner edge distance from beam centerline
    double cutout_dim_2;        //cutout dimension where block shear is measured
    ...
    //variables for gusset flexure yielding check at connection 2
    double ec_c2;               //eccentricity of force at connection 2
    double Mn_f_c2;             //Nominal flexure strength of gusset at connection 2
    double cap_f_c2;            //Gusset plate flexure strength at connection 2
	double ratio_f_c2;          //interaction ratio in gusset flexure at connection 2
    ...
    //variable for shear tab flexure yielding check at connection 2
    double Mn_fe_c2;            //nominal flexural strength of shear tab
    double cap_fe_c2;           //shear tab flexural strength  
    double ratio_fe_c2;         //interaction ratio in shear tab flexure
...
brace(section_property::MEMB_TYPE_L), clip(section_property::MEMB_TYPE_L),plate(section_property::MEMB_TYPE_PLATE){	
    ...
    if (data["BRACE_NUMBER_OF_BOLT_ROWS"] == "2")
        addParameter("BOLT_ROW_SPACING");
    else
        addParameter("BOLT_ROW_SPACING","0");
	param("BOLT_ROW_SPACING").isPositiveNumber();
    if (data["BACK_TO_BACK_LEG_ORIENTATION"] == "VERTICAL")
        addParameter("BRACE_ANGLE_SPACING");
    else
        addParameter("BRACE_ANGLE_SPACING","0");
	param("BRACE_ANGLE_SPACING").isPositiveNumber();	
    ...
    if (data["CONNECTION1_TYPE"] == "BOTTOM_FLANGE"){
        addParameter("CONNECTION1_EXTENSION");
        addParameter("CONNECTION1_SETBACK","0");		
    }else{
        addParameter("CONNECTION1_EXTENSION","0");
        addParameter("CONNECTION1_SETBACK");	
    }
    param("CONNECTION1_EXTENSION").isPositiveNumber();
	param("CONNECTION1_SETBACK").isPositiveNumber();
    if (data["CONNECTION2_TYPE"] == "BOTTOM_FLANGE"){
        addParameter("CONNECTION2_EXTENSION");        
        addParameter("CONNECTION2_SETBACK","0");
    }else{
        addParameter("CONNECTION2_EXTENSION","0");        
	...
	if (data["CONNECTION1_TYPE"] == "BOTTOM_FLANGE")
		addParameter("BOLT_GAGE_ON_BEAM1");	
	else
		addParameter("BOLT_GAGE_ON_BEAM1","0");	
	if (data["CONNECTION2_TYPE"] == "BOTTOM_FLANGE")
		addParameter("BOLT_GAGE_ON_BEAM2");	
	else
		addParameter("BOLT_GAGE_ON_BEAM2","0");	
	param("BOLT_GAGE_ON_BEAM1").isPositiveNumber();
	param("BOLT_GAGE_ON_BEAM2").isPositiveNumber();
    
    addParameter("JOINT_TYPE");
    param("JOINT_TYPE").addAllowableValue("BEAM_JOINT");
    param("JOINT_TYPE").addAllowableValue("COLUMN_JOINT");
    
    if((data.find("JOINT_TYPE")!= data.end()) && (data.at("JOINT_TYPE")=="COLUMN_JOINT"))
        addParameter("COLUMN_SECTION");
    else
        addParameter("COLUMN_SECTION","0");
    
    if((data.find("JOINT_TYPE")!= data.end()) && (data.at("JOINT_TYPE")=="BEAM_JOINT"))
        addParameter("CONTINUOUS_BEAM_NUMBER");
    else
        addParameter("CONTINUOUS_BEAM_NUMBER","0");
    param("CONTINUOUS_BEAM_NUMBER").addAllowableValue("1");
    param("CONTINUOUS_BEAM_NUMBER").addAllowableValue("2");
    addParameter("BEAM1_COPE_LENGTH","0");
    param("BEAM1_COPE_LENGTH").setMinimumValue(0);
    if((data.find("CONTINUOUS_BEAM_NUMBER") != data.end()) && (data.at("CONTINUOUS_BEAM_NUMBER") == "2"))
        addParameter("BEAM1_SETBACK");
    else        
        addParameter("BEAM1_SETBACK","0");         
    param("BEAM1_SETBACK").setMinimumValue(0);    
    addParameter("BEAM2_COPE_LENGTH","0");
    param("BEAM2_COPE_LENGTH").setMinimumValue(0);
    if((data.find("CONTINUOUS_BEAM_NUMBER") != data.end()) && (data.at("CONTINUOUS_BEAM_NUMBER") == "1"))
        addParameter("BEAM2_SETBACK");    
    else       
        addParameter("BEAM2_SETBACK","0"); 
    param("BEAM2_SETBACK").setMinimumValue(0);    
    if((data.find("JOINT_TYPE")!= data.end()) && (data.at("JOINT_TYPE")=="COLUMN_JOINT"))
        addParameter("COLUMN_ORIENTATION");
	else
        addParameter("COLUMN_ORIENTATION","0");
    param("COLUMN_ORIENTATION").addAllowableValue("0");
    param("COLUMN_ORIENTATION").addAllowableValue("90");
    ...
    sa = atof(data["BRACE_ANGLE_SPACING"].c_str());
    ...
    extension_c1 = atof(data["CONNECTION1_EXTENSION"].c_str());
    extension_c2 = atof(data["CONNECTION2_EXTENSION"].c_str());
	...
	if (conn1 == BOTTOM_FLANGE)
		gage_bm1 = atof(data["BOLT_GAGE_ON_BEAM1"].c_str());			
	if (conn2 == BOTTOM_FLANGE)
		gage_bm2 = atof(data["BOLT_GAGE_ON_BEAM2"].c_str());
    
    if ((data.find("JOINT_TYPE")!= data.end()) && (data.at("JOINT_TYPE")=="BEAM_JOINT"))
        jointType = BEAM_JOINT;
    else if ((data.find("JOINT_TYPE")!= data.end()) && (data.at("JOINT_TYPE")=="COLUMN_JOINT")){
        jointType = COLUMN_JOINT;    
        column.setSection(data["COLUMN_SECTION"]);
        d_col = column.depth();
        tw_col = column.webThickness();
        tf_col = column.flangeThickness();
        bf_col = column.flangeWidth();    
    }
    cont_beam = atoi(data["CONTINUOUS_BEAM_NUMBER"].c_str());
    cope_bm1 = atof(data["BEAM1_COPE_LENGTH"].c_str());
    setback_bm1 = atof(data["BEAM1_SETBACK"].c_str());
    cope_bm2 = atof(data["BEAM2_COPE_LENGTH"].c_str());
    setback_bm2 = atof(data["BEAM2_SETBACK"].c_str());
	
    col_orientation = atoi(data["COLUMN_ORIENTATION"].c_str());
...
void HBConnection1::checkFitup(){
	...
	if (conn1 == BOTTOM_FLANGE)
		if (0.5*bf_c1 - gage_bm1 < ed_min) err.addError("Minimum edge distance for bolts is not satisfied at beam flange of connection 1. Try reducing the gage on beam flange or increasing the flange width.");
    if (conn2 == BOTTOM_FLANGE)
		if (0.5*bf_c2 - gage_bm2 < ed_min) err.addError("Minimum edge distance for bolts is not satisfied at beam flange of connection 2. Try reducing the gage on beam flange or increasing the flange width.");
    //bolt clash with beam fillet area
    if (conn1 == BOTTOM_FLANGE)
        if (gage_bm1 < kdet + db) err.addError ("Insufficient bolt clearance from beam fillet area at connection 1. Try increasing bolt gage.");
    if (conn2 == BOTTOM_FLANGE)
        if (gage_bm2 < kdet + db) err.addError ("Insufficient bolt clearance from beam fillet area at connection 2. Try increasing bolt gage.");    
    ...
    if (conn1 == SHEAR_PLATE) {    
		...
		if (tog - ts < 0.5 * bf_c1) err.addError("Inadequate clearance of shear tab from top flange of beam at connection 1. Try increasing the value of the gusset top offset.");	
		//connection bolting clearance from bottom flange
		if (d_c1 - tog < 0.5 * bf_c1) err.addError("Inadequate clearance of shear tab from bottom flange of beam at connection 1. Try reducing the value of the gusset top offset or increasing the beam size.");
    }    
    
    if (conn2 == SHEAR_PLATE) {    
		//connection bolting clearance from top flange
		if (tog - ts < 0.5 * bf_c2) err.addError("Inadequate clearance of shear tab from top flange of beam at connection 2. Try increasing the value of the gusset top offset.");	
		//connection bolting clearance from bottom flange
		if (d_c2 - tog < 0.5 * bf_c2) err.addError("Inadequate clearance of shear tab from bottom flange of beam at connection 2. Try reducing the value of the gusset top offset or increasing the beam size.");
    }  
...
void HBConnection1::calc(){
	...
	if (brtype == DOUBLE_ANGLE_BRACE_BBLV)		
        ...
        nb = n_br;
	Rn_bv = nb*nr*blt.nominalShearStrength(nv);
	...
	A=na*brace.area(); 						            			//cross section area
    if (outstandingLeg == SHORT_LEG)
	...
	if (conn1 == BOTTOM_FLANGE){
        if (jointType == BEAM_JOINT){
            if (cont_beam != 1)
                ed_b1 = loc_c1 - cope_bm1 - setback_bm1 - 0.5*tw_c2;         //bolt edge distance on beam 1
            else
                ed_b1 = 9e20; //continuous beam - no edge
        }else{
            if (col_orientation == 0) //column web parellel to beam 1
                ed_b1 = loc_c1 - cope_bm1 - setback_bm1 - 0.5*d_col; 
            else
                ed_b1 = loc_c1 - cope_bm1 - setback_bm1 - 0.5*tw_col;
        }
		lcf_c1 = min(ed_b1 - 0.5*dbh, sb - dbh);
	...
	if (conn2 != BOTTOM_FLANGE)
        gus_in_ed_2 = setback_c2 + tw_c2/2;                             //gusset inner edge distance from beam centerline
    else
        gus_in_ed_2 = -extension_c2;
    if(conn1 == SHEAR_PLATE){ 
        gedo_c1 = g_c1 + gus_in_ed_2;							        //distance of outer edge of gusset from connection work point
        cutout_dim_1 = (c_c2==0?0:(gage_t - setback_c1)*c_c1/c_c2);     //cutout dimension where block shear is measured
        gedi_c1 = c_c1 + gus_in_ed_2 - cutout_dim_1;			        //distance of inner edge of gusset from connection work point 
        ...
        gedo_c1 = g_c1 + gus_in_ed_2;							        //distance of outer edge of gusset from connection work point
        cutout_dim_1 = (c_c2==0?0:(il_c - setback_c1)*c_c1/c_c2);       //cutout dimension where block shear is measured
        gedi_c1 = c_c1 + gus_in_ed_2 - cutout_dim_1;			        //distance of inner edge of gusset from connection work point 
        ...
        gedo_c1 = g_c1 + gus_in_ed_2;							    	//distance of outer edge of gusset from connection work point
        cutout_dim_1 = (c_c2==0?0:(gage_bm1 + extension_c1)*c_c1/c_c2); //cutout dimension where block shear is measured
        gedi_c1 = c_c1 + gus_in_ed_2 - cutout_dim_1;			        //distance of inner edge of gusset from connection work point 
		l_c1 = (n1 - 1)*sb;
		...
		Agv_bsg_c1 = ((n1-1)*sb + ed_c1) * tg;							//gross area in shear for block shear
		Anv_bsg_c1 = Agv_bsg_c1 - (n1-0.5) * dbh * tg;					//net area in shear for block shear
		Ant_bsg_c1 = (gage_bm1 + extension_c1 - 0.5*dbh) * tg;          //net area in tension for block shear
	}
    ...
    //Gusset flexure yielding check at connection 1
    if (conn1 != BOTTOM_FLANGE)
        ec_c1 = c_c2 + setback_c1 + tw_c1/2;                            //eccentricity of force at connection 1
    else
        ec_c1 = c_c2 - extension_c1;
    ec_c1 = ec_c1>0?ec_c1:0;
    Mn_f_c1 = Fy_g*tg*g_c1*g_c1/4;                                      //Nominal flexure strength of gusset at connection 1
    if (met == LRFD)
        cap_f_c1 = phi_f*Mn_f_c1;
    else
		cap_f_c1 = Mn_f_c1/omega_f;
	ratio_f_c1 = P_c1*ec_c1 / cap_f_c1;        
    ...
    //shear tab flexure yielding check
    if (conn1 == SHEAR_PLATE){
        Mn_fe_c1 = Fy_g*ts*l_c1*l_c1/4;                                  //nominal flexural strength of shear tab
        if (met == LRFD) 
            cap_fe_c1 = phi_f*Mn_fe_c1; 
        else 
            cap_fe_c1 = Mn_fe_c1/omega_f;   
        ratio_fe_c1 = P_c1*gage_t / cap_fe_c1;	
    }
	...
	if (conn1 != BOTTOM_FLANGE){
		...
		if (conn1 == SHEAR_PLATE){
			...
			ratio_rww_c1 = f_w_c1 / cap_rww_c1;
	...
	if (conn2 == BOTTOM_FLANGE){
        if (jointType == BEAM_JOINT){
            if (cont_beam != 2)
                ed_b2 = loc_c2 - cope_bm2 - setback_bm2 - 0.5*tw_c1;         //bolt edge distance on beam 2 bottom flange
            else
                ed_b2 = 9e20; //continuous beam - no edge
        }else{
            if (col_orientation == 90) //column web parellel to beam 2
                ed_b2 = loc_c2 - cope_bm2 - setback_bm2 - 0.5*d_col; 
            else
                ed_b2 = loc_c2 - cope_bm2 - setback_bm2 - 0.5*tw_col;
        }
        lcf_c2 = sb-dbh;
    ...
    if (conn1 != BOTTOM_FLANGE)
        gus_in_ed_1 = setback_c1 + tw_c1/2;                             //gusset inner edge distance from beam centerline
    else
        gus_in_ed_1 = -extension_c1;
	if(conn2 == SHEAR_PLATE){
        gedo_c2 = g_c2 + gus_in_ed_1;							        //distance of outer edge of gusset from connection work point
        cutout_dim_2 = (c_c1==0?0:(gage_t - setback_c2)*c_c2/c_c1);     //cutout dimension where block shear is measured
        gedi_c2 = c_c2 + gus_in_ed_1 - cutout_dim_2;			        //distance of inner edge of gusset from connection work point        
        ...
        gedo_c2 = g_c2 + gus_in_ed_1;							        //distance of outer edge of gusset from connection work point
        cutout_dim_2 = (c_c1==0?0:(il_c - setback_c2)*c_c2/c_c1);       //cutout dimension where block shear is measured
        gedi_c2 = c_c2 + gus_in_ed_1 - cutout_dim_2;			        //distance of inner edge of gusset from connection work point
        ...
        gedo_c2 = g_c2 + gus_in_ed_1;							    	//distance of outer edge of gusset from connection work point
        cutout_dim_2 = (c_c1==0?0:(gage_bm2 + extension_c2)*c_c2/c_c1); //cutout dimension where block shear is measured
        gedi_c2 = c_c2 + gus_in_ed_1 - cutout_dim_2;			        //distance of inner edge of gusset from connection work point 
		l_c2 = (n2 - 1)*sb;
		...
		Agv_bsg_c2 = ((n2-1)*sb + ed_c2) * tg;							//gross area in shear for block shear
		Anv_bsg_c2 = Agv_bsg_c2 - (n2-0.5) * dbh * tg;					//net area in shear for block shear
		Ant_bsg_c2 = (gage_bm2 + extension_c2 - 0.5*dbh) * tg;	        //net area in tension for block shear
	}
	...
	//bolt bearing at gusset at connection 2 
    ...
    //Gusset flexure yielding check at connection 2
    if (conn2 != BOTTOM_FLANGE)
        ec_c2 = c_c1 + setback_c2 + tw_c2/2;                                //eccentricity of force at connection 2
    else
        ec_c2 = c_c1 - extension_c2;
    ec_c2 = ec_c2>0?ec_c2:0;
    Mn_f_c2 = Fy_g*tg*g_c2*g_c2/4;                                      //Nominal flexure strength of gusset at connection 2
    if (met == LRFD)
        cap_f_c2 = phi_f*Mn_f_c2;
    else
		cap_f_c2 = Mn_f_c2/omega_f;
	ratio_f_c2 = P_c2*ec_c2 / cap_f_c2;   
    ...
    //shear tab flexure yielding check
    if (conn2 == SHEAR_PLATE){
        Mn_fe_c2 = Fy_g*ts*l_c2*l_c2/4;                                  //nominal flexural strength of shear tab
        if (met == LRFD) 
            cap_fe_c2 = phi_f*Mn_fe_c2; 
        else 
            cap_fe_c2 = Mn_fe_c2/omega_f;   
        ratio_fe_c2 = P_c2*gage_t / cap_fe_c2;	
    }
	...
	if (conn2 != BOTTOM_FLANGE){
		...
		if (conn2 == SHEAR_PLATE){
			...
			ratio_rww_c2 = f_w_c2 / cap_rww_c2;
			
			fn_rwt_c2 = 0.6 * Fu_g * ts;									//nominal rupture strength of shear tab at weld
			if (met == LRFD)
	...
	ratio_max = ratio_rg > ratio_max? ratio_rg : ratio_max;				//Interaction ratio in gusset tension rupture 
	...
	ratio_max = ratio_f_c1 > ratio_max? ratio_f_c1 : ratio_max;		    //interaction ratio for gusset flexure yielding at connection 1 
	if ((conn1 == BOTTOM_FLANGE)||(conn1 == SHEAR_PLATE))
	...
	if (conn1 == SHEAR_PLATE){
        ...
        ratio_max = ratio_fe_c1 > ratio_max? ratio_fe_c1 : ratio_max;	//Interation ratio in shear tab flexure at connection 1
	}
	...
	ratio_max = ratio_f_c2 > ratio_max? ratio_f_c2 : ratio_max;		    //interaction ratio for gusset flexure yielding at connection 2 
	if ((conn2 == BOTTOM_FLANGE)||(conn2 == SHEAR_PLATE))
	...
	if (conn2 == SHEAR_PLATE){
        ...
        ratio_max = ratio_fe_c2 > ratio_max? ratio_fe_c2 : ratio_max;	//Interation ratio in shear tab flexure at connection 2
	}
...
std::string HBConnection1::designSummary(){
    ...
    if (ratio_rg > 1.0) _p("","Fails in gusset tension rupture");
    ...
    if (ratio_f_c1 > 1.0) _p("","Fails in gusset flexure yielding at connection 1");
	if ((conn1 == BOTTOM_FLANGE)||(conn1 == SHEAR_PLATE))
	...
	if (conn1 == SHEAR_PLATE){
        ...
        if (ratio_fe_c1 > 1.0) _p("","Fails in shear tab flexure yielding at connection 1");
	}
	...
	if (conn2 == SHEAR_PLATE){
        ...
        if (ratio_fe_c2 > 1.0) _p("","Fails in shear tab flexure yielding at connection 2");
	}
...
std::string HBConnection1::designCalculation(){
	...
	//brace connection checks
    //Bolt shear check at brace
	_h3("Bolt shear at brace check");
	...
	_h3("Brace tension rupture check");
	...
	_p(An_g,"Gusset plate net area in tension","",_area);
	_p(Pn_rg,"Nominal strength in gusset rupture (Pn)","",_force);
	if (met == LRFD){
		_p(phi_rg,"LRFD factor in tension rupture","phi","");
		_p(cap_rg,"Allowable strength of gusset tension rupture","Pa=phi*Pn",_force);
	}else{
		_p(omega_rg,"ASD factor in tension rupture","omega","");
		_p(cap_rg,"Allowable strength of gusset in tension rupture","Pa=Pn/omega",_force);		
	}
	_p(ratio_rg,"Interaction ratio in gusset plate tension rupture","P/Pa","");
    ...
    //Bolt bearing at shear tab check (connection 1)
	if (conn1 == SHEAR_PLATE){	
	...
	//Bolt bearing at clip angle check (connection 1)
	if (conn1 == CLIP_ANGLE){	
    ...
    //Bolt bearing at beam web check (connection 1)
	if (conn1 == CLIP_ANGLE){	
    ...
    //Bolt bearing at gusset check (connection 1)
	if ((conn1 == BOTTOM_FLANGE)||(conn1 == SHEAR_PLATE)){
		_h3("Bolt bearing at gusset check");
    ...
    //Bolt bearing at beam flange check (connection 1)
	if (conn1 == BOTTOM_FLANGE){
    ...
    //Gusset shear yielding check (connection 1)
    ...
    //Gusset shear rupture check (connection 1)
	if (conn1 != CLIP_ANGLE){
    ...
    //Gusset plate block shear check (connection 1)
	_h3("Gusset plate block shear check");
    ...
    //Gusset flexure yielding check (connection 1)
	_h3("Gusset flexure yielding check");
	_p(ec_c1,"Eccentricity of force at connection (e)", "", _length);    
	_p(Mn_f_c1,"Nominal flexure strength of gusset in yielding (Mn)", "", _moment);
	if (met == LRFD){
		_p(phi_f,"LRFD factor in flexure yielding (phi)","","");
		_p(cap_f_c1,"Allowable flexure strength of gusset in yielding","Ma=phi*Mn",_moment);
	}else{
		_p(omega_f,"ASD factor in flexure yielding (omega)","","");
		_p(cap_f_c1,"Allowable flexure strength of gusset in yielding","Ma=Mn/omega",_moment);		
	}
	_p(ratio_f_c1,"Interaction ratio in flexure yielding at gusset", "P1*e/Ma","");  		
	//Shear tab/clip angle shear yielding check (connection 1)
	if (conn1 != BOTTOM_FLANGE){
		if (conn1==SHEAR_PLATE)
    ...
    //Shear tab/clip angle shear rupture check (connection 1)
    if (conn1 != BOTTOM_FLANGE){
		if(conn1==SHEAR_PLATE)
    ...
    //Shear tab/Clip angle block shear check (connection 1)	
	if (conn1 != BOTTOM_FLANGE){
		if (conn1 == SHEAR_PLATE)
    ...
    //Shear tab flexure yeilding check (connection 1)
    if(conn1==SHEAR_PLATE){
        _h3("Shear tab flexure yeilding check");
        _p(Mn_fe_c1,"Nominal flexure yeilding strength of connecting element (Mn)", "",_moment);
        if (met==LRFD){
            _p(phi_f,"LRFD factor in flexure (phi)","","");
            _p(cap_fe_c1,"Allowable strength of connecting element in flexure","Ma=phi*Mn",_moment);
        }else{
            _p(omega_f,"ASD factor in flexure (omega)","","");
            _p(cap_fe_c1,"Allowable strength of connecting element in flexure","Ma=Mn/omega",_moment);		
        }			
        _p(ratio_fe_c1, "Interaction ratio in flexure yielding of connecting element","P1*gs/Ma","");
    }
    //Weld check (connection 1)
    if (conn1 != BOTTOM_FLANGE){
		_h3("Weld check");
	...
	//Gusset rupture at weld check (connection 1)
    if (conn1 == CLIP_ANGLE){
	...
	//Shear tab rupture at weld check (connection 1)
    if (conn1 == SHEAR_PLATE){			
    ...
    //Beam web rupture at weld check (connection 1)
    if (conn1 == SHEAR_PLATE){		
    ...
    //Bolt bearing at shear tab check (connection 2)
	if (conn2 == SHEAR_PLATE){	
		_h3("Bolt bearing at shear tab check");
	...
	//Bolt bearing at clip angle check (connection 2)
	if (conn2 == CLIP_ANGLE){
		_h3("Bolt bearing at clip angle check");
    ...
    //Bolt bearing at beam web check (connection 2)
    if (conn2 == CLIP_ANGLE){
		_h3("Bolt bearing at beam web check");
    ...
    //Bolt bearing at gusset check (connection 2)
	if ((conn2 == BOTTOM_FLANGE)||(conn2 == SHEAR_PLATE)){
		_h3("Bolt bearing at gusset check");
    ...
    //Bolt bearing at beam flange check (connection 2)
	if (conn2 == BOTTOM_FLANGE){
		_h3("Bolt bearing at beam flange check");
    ...
    //Gusset shear yielding check (connection 2)
	_h3("Gusset shear yielding check");
	...
	//Gusset shear rupture check (connection 2)
	if (conn2 != CLIP_ANGLE){
		_h3("Gusset shear rupture check");
	...
	//Gusset block shear check (connection 2)
	_h3("Gusset block shear check");
	...
	//Gusset flexure yielding check (connection 2)
	_h3("Gusset flexure yielding check");
	_p(ec_c2,"Eccentricity of force at connection (e)", "", _length);    
	_p(Mn_f_c2,"Nominal flexure strength of gusset in yielding (Mn)", "", _moment);
	if (met == LRFD){
		_p(phi_f,"LRFD factor in flexure yielding (phi)","","");
		_p(cap_f_c2,"Allowable flexure strength of gusset in yielding","Ma=phi*Mn",_moment);
	}else{
		_p(omega_f,"ASD factor in flexure yielding (omega)","","");
		_p(cap_f_c2,"Allowable flexure strength of gusset in yielding","Ma=Mn/omega",_moment);		
	}
	_p(ratio_f_c2,"Interaction ratio in flexure yielding at gusset", "P1*e/Ma",""); 			     
    //Shear tab / clip angle shear yielding check (connection 2)
	if (conn2 != BOTTOM_FLANGE){
		if (conn2==SHEAR_PLATE)
			_h3("Shear tab shear yielding check");
		else
    ...
    //Shear tab / clip angle shear rupture check (connection 2)
    if (conn2 != BOTTOM_FLANGE){
		if(conn2==SHEAR_PLATE)
			_h3("Shear tab shear rupture check");
		else
	...
	//Shear tab/ clip angle block shear check (connection 2)
	if (conn2 != BOTTOM_FLANGE){
		if (conn2 == SHEAR_PLATE)
			_h3("Shear tab block shear check");
		else if (conn2 == CLIP_ANGLE)
    ...
    //Shear tab flexure yeilding check (connection 2)
    if(conn2==SHEAR_PLATE){
        _h3("Shear tab flexure yeilding check");
        _p(Mn_fe_c2,"Nominal flexure yeilding strength of connecting element (Mn)", "",_moment);
        if (met==LRFD){
            _p(phi_f,"LRFD factor in flexure (phi)","","");
            _p(cap_fe_c2,"Allowable strength of connecting element in flexure","Ma=phi*Mn",_moment);
        }else{
            _p(omega_f,"ASD factor in flexure (omega)","","");
            _p(cap_fe_c2,"Allowable strength of connecting element in flexure","Ma=Mn/omega",_moment);		
        }			
        _p(ratio_fe_c2, "Interaction ratio in flexure yielding of connecting element","P2*gs/Ma","");
    }
    //Weld check (connection 2)
    if (conn2 != BOTTOM_FLANGE){
        _h3("Weld check");
    ...
    //Gusset rupture at weld check (connection 2)
    if (conn2 == CLIP_ANGLE){
        _h3("Gusset rupture at weld check");
	...
	//Shear tab rupture at weld check (connection 2)
    if (conn2 == SHEAR_PLATE){			
        _h3("Shear tab rupture at weld check");
    ...
    //Beam web rupture at weld check (connection 2)
	if (conn2 == SHEAR_PLATE){	
        _h3("Beam web rupture at weld check");
